...
public class ErlangEmacsFormatAction extends AnAction implements DumbAware
{
	...
	public void actionPerformed(final AnActionEvent e)
	{
		...
		try
		{
			...
			{
				Notifications.Bus.notify(new Notification(groupId, NOTIFICATION_TITLE, "Emacs executable path is empty" + "<br/><a " +
						"href='configure'>Configure</a>", NotificationType.WARNING, new ErlangExternalToolsNotificationListener(project)), project);
				return;
			}
			commandLine.setExePath(emacsPath);
			commandLine.addParameters("--batch", "--eval");

			String sdkPath = getSdkPath(psiFile);

			if(StringUtil.isEmpty(sdkPath))
			{
				Notifications.Bus.notify(new Notification(groupId, NOTIFICATION_TITLE, "Erlang project SDK is not configured",
			...
			{
				Notifications.Bus.notify(new Notification(groupId, NOTIFICATION_TITLE, "Failed to create a temporary file",
						NotificationType.WARNING), project);
				return;
			}
			...
			String emacsCommand = "\n" +
					...
					"    (setq load-path (cons (car (file-expand-wildcards (concat erlang-root-dir \"/lib/" + (exists ? "erlang/lib/" : "") +
					"tools-*/emacs\")))\n" +
					"                          load-path))\n" +
					...
					ApplicationManager.getApplication().invokeLater(new Runnable()
					{
						...
						public void run()
						{
							try
							{
								...
								{
									Notifications.Bus.notify(new Notification(groupId, NOTIFICATION_TITLE, "Emacs returned an empty file",
											NotificationType.WARNING), project);
								...
								Notifications.Bus.notify(new Notification(groupId, NOTIFICATION_TITLE, psiFile.getName() + " formatted with Emacs",
										NotificationType.INFORMATION), project);

							}
							catch(Exception ex)
							{
								Notifications.Bus.notify(new Notification(groupId, psiFile.getName() + " formatting with Emacs failed",
										ExceptionUtil.getUserStackTrace(ex, LOG), NotificationType.ERROR), project);
								LOG.error(ex);
							}
						}
					});
