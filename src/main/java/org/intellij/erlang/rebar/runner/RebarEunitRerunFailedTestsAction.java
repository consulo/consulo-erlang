	...
	public MyRunProfile getRunProfile(@NotNull ExecutionEnvironment executionEnvironment)
	{
		...
		return new MyRunProfile((RunConfigurationBase) model.getProperties().getConfiguration())
		{
			...
			private RebarEunitRunConfiguration createRerunFailedTestsRunConfiguration()
			{
				...
				List<ErlangFunction> failedTests = ContainerUtil.mapNotNull(getFailedTests(project), new Function<AbstractTestProxy,
						ErlangFunction>()
				{
					@Nullable
					@Override
			...
			private void notifyGeneratedTestsFailed(final List<ErlangFunction> failedGeneratedTests)
			{
				ApplicationManager.getApplication().invokeLater(new Runnable()
				{
					public void run()
					{
						Notifications.Bus.notify(new Notification("TestRunner", "Some tests cannot be rerun directly",
