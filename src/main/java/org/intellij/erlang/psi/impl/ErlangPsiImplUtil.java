...
public class ErlangPsiImplUtil
{
	...
	public static final Set<String> BUILT_IN_TYPES = ContainerUtil.set("any", "atom", "boolean", "byte", "char", "float", "integer", "iolist", "list", "maybe_improper_list", "mfa", "module",
			"neg_integer", "no_return", "node", "non_neg_integer", "none", "nonempty_string", "number", "pid", "port", "pos_integer", "ref", "string", "term", "timeout");
	...
	private static PsiReference getRecordFieldReference(@Nullable ErlangQAtom atom)
	{
		...
		return new ErlangAtomBasedReferenceImpl<ErlangQAtom>(atom, TextRange.from(0, atom.getTextLength()), atom.getText())
		{
			...
			public PsiElement resolve()
			{
				...
				{
					if(field.getName().equals(myReferenceName))
					{
						return field;
					}
				}
				for(ErlangQAtom qAtom : recordFields.second)
				{
					...
					if(aa != null)
					{
						if(myReferenceName.equals(aa.getText()))
						{
							return qAtom;
						}
					}
	...
	public static PsiReference getReference(@Nonnull ErlangFunctionCallExpression o)
	{
		...
		return new ErlangFunctionReferenceImpl<ErlangQAtom>(nameAtom, moduleAtom, TextRange.from(0, nameAtom.getTextLength()), nameAtom.getText(), o.getArgumentList().getExpressionList().size());
	}
	...
	public static PsiReference getReference(@Nonnull ErlangFunctionWithArity o)
	{
		...
		if(moduleReference != null && moduleReference.getQAtom().getMacros() != null && !isModule)
		{
			return null;
		}
		ErlangQAtom moduleAtom = moduleReference == null ? null : moduleReference.getQAtom();
		...
		return new ErlangFunctionReferenceImpl<ErlangQAtom>(nameAtom, isModule ? null : moduleAtom, TextRange.from(0, nameAtom.getTextLength()), nameAtom.getText(), getArity(arity));
	}
	...
	{
		return inArgumentDefinition(psiElement) && inArgumentList(psiElement) && PsiTreeUtil.getParentOfType(psiElement, ErlangArgumentDefinition.class,
				ErlangArgumentList.class) instanceof ErlangArgumentDefinition;
	}
	...
	{
		ErlangArgumentList argList = PsiTreeUtil.getParentOfType(psiElement, ErlangArgumentList.class, true, ErlangFunctionCallExpression.class, ErlangFunClause.class, ErlangListComprehension.class);
	...
	public static boolean inLeftPartOfAssignment(@Nonnull PsiElement psiElement, boolean strict)
	{
		...
		ErlangAssignmentExpression topAssignment = insideTupleOrList ? PsiTreeUtil.getParentOfType(parent, ErlangAssignmentExpression.class) : null;
	...
	public static List<LookupElement> getFunctionLookupElements(@Nonnull PsiFile containingFile, final boolean withArity, @Nullable ErlangQAtom qAtom)
	{
		if(containingFile instanceof ErlangFile && !ErlangParserUtil.isApplicationConfigFileType(containingFile))
		{
			...
			else
			{
				...
				{
					lookupElements.add(createFunctionLookupElement(importFunction.getQAtom().getText(), getArity(importFunction.getInteger()), withArity,
							ErlangCompletionContributor.MODULE_FUNCTIONS_PRIORITY));
				}
	...
	{
		return withArity ? new BasicInsertHandler<LookupElement>()
		{
			@Override
	...
	public static List<LookupElement> getTypeLookupElements(@Nonnull PsiFile containingFile, boolean addBuiltInTypes, final boolean withArity)
	{
		if(containingFile instanceof ErlangFile)
		{
			...
			List<LookupElement> builtInTypes = addBuiltInTypes ? ContainerUtil.map(BUILT_IN_TYPES, new Function<String, LookupElement>()
			{
				...
				{
					return PrioritizedLookupElement.withPriority(LookupElementBuilder.create(s).withIcon(ErlangIcons.TYPE).withInsertHandler(handler), ErlangCompletionContributor.TYPE_PRIORITY);
				}
	...
	public static int getTextOffset(@Nonnull ErlangRecordDefinition o)
	{
		...
		{
			return 0;//o.getNode().getTextOffset();
		}
	...
	public static List<ErlangFile> getDirectlyIncludedFiles(@Nullable ErlangIncludeString includeString, @Nonnull ErlangFile erlangFile)
	{
		...
		if(relativeToDirectParent != null)
		{
			return ContainerUtil.newSmartList(relativeToDirectParent);
		}
	...
	private static List<ErlangFile> getDirectlyIncludedFilesForSmallIde(@Nonnull Project project, @Nonnull String includeStringPath, @Nullable VirtualFile otpAppRoot)
	{
		...
		if(relativeToOtpIncludeDirectory != null)
		{
			return ContainerUtil.newSmartList(relativeToOtpIncludeDirectory);
		}
	...
	public static void extractParseTransforms(@Nonnull ErlangTupleExpression tuple, @Nonnull Set<String> parseTransforms)
	{
		...
		if(!"parse_transform".equals(getAtomName(first instanceof ErlangMaxExpression ? (ErlangMaxExpression) first : null)))
		{
			return;
		}
	...
	public static PsiReference getReference(ErlangSpecFun o)
	{
		...
		if(arity != null)
		{
			return new ErlangFunctionReferenceImpl<ErlangQAtom>(atom, moduleRef == null ? null : moduleRef.getQAtom(), TextRange.from(0, atom.getTextLength()), atom.getText(), arity);
		}
		return null;
	}
	...
	public static Integer getArity(@Nonnull ErlangSpecFun o)
	{
		...
		if(integer != null)
		{
			arity = getArity(integer);
		}
		ErlangTypeSig sigs = PsiTreeUtil.getNextSiblingOfType(o, ErlangTypeSig.class);
		if(arity == null && sigs != null)
		{
			arity = sigs.getFunType().getFunTypeArguments().getTopTypeList().size();
		}
		return arity;
	}
	...
	{
		return isEunitDirectlyImported(file) || ContainerUtil.find(getIncludedFiles(file), new Condition<ErlangFile>()
		{
			@Override
	...
	{ // todo: use ref search
		...
		if(prevSibling instanceof ErlangFunction)
		{
			return null;
		}
	...
	{
		if(element instanceof ErlangCatchExpression)
		{
			return 0;
		}
		...
		if(element instanceof ErlangSendExpression)
		{
			return 2;
		}
		if(element instanceof ErlangOrelseExpression)
		{
			return 3;
		}
		if(element instanceof ErlangAndalsoExpression)
		{
			return 4;
		}
		if(element instanceof ErlangCompOpExpression)
		{
			return 5;
		}
		...
		if(element instanceof ErlangAdditiveExpression)
		{
			return 7;
		}
		if(element instanceof ErlangMultiplicativeExpression)
		{
			return 8;
		}
		if(element instanceof ErlangPrefixExpression)
		{
			return 9;
		}
		if(element instanceof ErlangColonQualifiedExpression)
		{
			return 10;
		}
		if(element instanceof ErlangFunctionCallExpression || element instanceof ErlangGlobalFunctionCallExpression || element instanceof ErlangGenericFunctionCallExpression || element instanceof ErlangAnonymousCallExpression || element instanceof ErlangRecordExpression || element instanceof ErlangQualifiedExpression)

		{
			return 11;
		}
		if(element instanceof ErlangMaxExpression || element instanceof ErlangTupleExpression || element instanceof ErlangListExpression || element instanceof ErlangCaseExpression || element instanceof ErlangIfExpression || element instanceof ErlangListComprehension || element instanceof ErlangReceiveExpression || element instanceof ErlangFunExpression || element instanceof ErlangTryExpression || element instanceof ErlangBinaryExpression || element instanceof ErlangBeginEndExpression)
		{
			return 12;
		}
		if(element instanceof ErlangParenthesizedExpression)
		{
			return 13;
		}
		return -1;
	...
	public static ErlangExpression getOutermostParenthesizedExpression(@Nonnull ErlangExpression expression)
	{
		while(expression.getParent() instanceof ErlangParenthesizedExpression)
		{
			...
			if(!expression.isEquivalentTo(parent.getExpression()))
			{
				break;
			}
			expression = parent;
	...
	public static class ErlangFunctionCallParameter<T extends PsiElement> extends PatternCondition<T>
	{
		...
		public boolean accepts(@Nonnull T element, ProcessingContext context)
		{
			...
			if(list instanceof ErlangArgumentList)
			{
				...
				if(!(expressions.size() > myPosition && expressions.get(myPosition) == expr))
				{
					return false;
				}
				if(funCall instanceof ErlangFunctionCallExpression)
				{
					...
					ResolveResult[] results = reference instanceof PsiPolyVariantReference ? ((PsiPolyVariantReference) reference).multiResolve(true) : ResolveResult.EMPTY_ARRAY;
					for(ResolveResult r : results)
					{
						if(expectedFunction(r.getElement()))
						{
							return true;
						}
					}
